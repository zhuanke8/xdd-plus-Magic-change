...
func initFarm(cookie string, state chan string) {
	...
	req2 := httplib.Post(`https://api.m.jd.com/client.action?functionId=taskInitForFarm`)
	req2.Header("accept", "*/*")
	req2.Header("accept-encoding", "gzip, deflate, br")
	req2.Header("accept-language", "zh-CN,zh;q=0.9")
	req2.Header("cache-control", "no-cache")
	req2.Header("cookie", cookie)
	req2.Header("origin", "https://home.m.jd.com")
	req2.Header("pragma", "no-cache")
	req2.Header("referer", "https://home.m.jd.com/myJd/newhome.action")
	req2.Header("sec-fetch-dest", "empty")
	req2.Header("sec-fetch-mode", "cors")
	req2.Header("sec-fetch-site", "same-site")
	req2.Header("User-Agent", ua)
	req2.Header("Content-Type", "application/x-www-form-urlencoded")
	req2.Body(`body={"version":14,"channel":1,"babelChannel":"120"}&appid=wh5&clientVersion=9.1.0`)
	data2, _ := req2.Bytes()
	waterEveryDayT, _ := jsonparser.GetInt(data2, "totalWaterTaskInit", "totalWaterTaskTimes")
